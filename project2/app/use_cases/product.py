...
class ProductUseCases:
    ...
    def add_product(self, product : Product,category_slug: str):
        ...
        if not category : 
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"No category was found at {category_slug}")
    ...
    def update_product(self,id: int, product:Product):
        ...
        if product_on_db is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="No product was found at {product_on_db.id}")
    ...
    def list_products(self,search: str = ""):
        ...
        products = [
            self.serialize_product(product_on_db) for product_on_db in products_on_db
        ]
        return products
